...
// In these tests, stores and productStores are unused:
...
describe("GET /brands/:brandId/products", () => {
  ...
  beforeEach(async () => {
    ...
    const repo = new InMemoryRepository(
      // Cast to mutable arrays because InMemoryRepository expects plain arrays
      [...mockBrands] as any,
      [...mockProducts] as any,
      [...mockStores] as any,
      { ...mockProductStores }
  ...
  it("returns 200 & paginated list of products (own + consolidated) for an existing brand", async () => {
    ...
    // The two products (prod-1 and prod-3) should both appear, in any order:
  ...
  it("treats invalid page/per_page as 1", async () => {
    ...
    // Both “page” and “per_page” should clamp to 1
    expect(body.meta.page).toBe(1);
    expect(body.meta.itemsPerPage).toBe(1);

    // Must still return an array (at least the first product)
